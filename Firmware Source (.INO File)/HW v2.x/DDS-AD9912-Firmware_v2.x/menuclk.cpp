...
#define DBG 0
  ...
  String ClockSourceName[4]={"XO AT-cut", "TCXO", "ExtTCXO/OCXO", "REF CLK IN"};
...
void DDS_Clock_Config_Menu()
{
  ...
  if (EditMode == false)
  {
   ...
   /* if (incrValue != 0)  // ускоренная перемотка //curPos
    ...
    if (incrValue != 0)
    {
      switch (SetupMenuPos)
        ...
        case CLOCK_MENU_FREQ_INDEX: 
          ...
          if (ClockSourceIndex == CLOCK_SOURCE_EXT_TCXO_INDEX) {TCXO_Freq_Index++; FindBestNMultiplier();}
    ...
    if (ClockSourceIndex == CLOCK_SOURCE_EXT_TCXO_INDEX) Ref_Clk=Ref_Clk=TCXO_Values[TCXO_Freq_Index]; // и это
...
uint32_t GetDDSCoreClock()
{
  ...
  if ((ClockSourceIndex == CLOCK_SOURCE_XO_INDEX)  || 
      ...
      (ClockSourceIndex == CLOCK_SOURCE_EXT_TCXO_INDEX)) 
       {
